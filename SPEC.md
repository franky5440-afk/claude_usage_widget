...
- **D Session Context**：主 session 顯示為 `Dispatch`、子 session 為 `Dispatch 子任務`；context＝最後一則 `parent_tool_use_id` 為 null 的 assistant usage；分母依最後一筆 `system.model`，規則同 §11.1（Opus 5／Fable 5 系列不帶 `[1m]` 也是 1M；2026-09-15 實機套 200K 曾算出 110.3%）；查不到→null。與 CLI session 一起依 mtime 排序、共用 5 分鐘窗口與 3 條上限；窗口外的檔不得開啟。
